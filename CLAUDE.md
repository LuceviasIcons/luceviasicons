...
Положить SVG в `packages/core/svg/`. Имя файла = имя иконки; вес задаётся
суффиксом (`bell-bold.svg`) или папкой (`bold/bell.svg`). Веса: `thin`, `light`,
`regular`, `bold`, `fill`, `duotone`; отсутствующий вес откатывается на regular.
