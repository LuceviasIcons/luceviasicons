...
`packages/core/history.json` помнит, в какой версии иконка появилась и когда
последний раз менялась её разметка. Файл ведёт `scripts/build-history.mjs`
(входит в `core:build`) и дописывает, а не пересобирает: версия появления —
исторический факт, из текущей папки его не вычислить. Поэтому файл лежит в
гите, а после релиза workflow возвращает обновлённый вариант в `main`.

Сайт по этим данным показывает метку: иконка из последнего релиза считается
новой, из предыдущих — обычной. Статус одинаков для всех посетителей и
гаснет сам со следующим релизом.
